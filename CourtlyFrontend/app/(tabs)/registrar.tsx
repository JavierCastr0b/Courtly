...
import type { LevelType } from '@/src/data/mockData';
import { Button } from '@/src/components/Button';
import { Tag } from '@/src/components/Tag';

type Mode = 'match' | 'post';

const LEVELS: LevelType[] = ['Principiante', 'Intermedio', 'Avanzado'];
...
export default function RegistrarScreen() {
  ...
  const [selectedLevel, setSelectedLevel] = useState<LevelType>('Intermedio');
        ...
        <View style={styles.modeSwitcher}>
          ...
          >
            <Ionicons
              ...
              size={15}
              color={mode === 'match' ? colors.primary : colors.textSecondary}
            />
            <Text style={[styles.modeBtnText, mode === 'match' && styles.modeBtnTextActive]}>
              Partido
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            ...
            />
            <Text style={[styles.modeBtnText, mode === 'post' && styles.modeBtnTextActive]}>
              Publicación
            </Text>
          </TouchableOpacity>
        </View>
            ...
            <>
              ...
              {mockCourts.map((c) => (
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.courtName, selectedCourt === c.id && { color: colors.primary }]}>
                      {c.name}
                    </Text>
                    <Text style={styles.courtMeta}>{c.address} · {c.distance}</Text>
                  </View>
                  {selectedCourt === c.id && (
              ...
              <View style={styles.levelRow}>
                {LEVELS.map((lvl) => (
                  <TouchableOpacity
                    ...
                    style={[styles.levelCard, selectedLevel === lvl && styles.levelCardActive]}
                    activeOpacity={0.75}
                  >
                    <Tag
                      label={lvl}
              ...
              {court && (
                <View style={styles.preview}>
                  ...
                  <View style={styles.previewRow}>
                    ...
                    <Text style={styles.previewText}>{players} jugadores · {selectedLevel}</Text>
                  </View>
                </View>
              )}
              ...
              <View style={styles.levelRow}>
                {LEVELS.map((lvl) => (
                  <TouchableOpacity
                    ...
                    style={[styles.levelCard, selectedLevel === lvl && styles.levelCardActive]}
                  >
                    <Tag label={lvl} variant="level" level={lvl} />
                  </TouchableOpacity>
                ))}
              </View>

              <Text style={styles.sectionLabel}>Ubicación (opcional)</Text>
              {mockCourts.slice(0, 3).map((c) => (
                <TouchableOpacity
                  key={c.id}
                  onPress={() => setSelectedCourt(c.id)}
                  style={[styles.courtItem, selectedCourt === c.id && styles.courtItemSelected]}
                >
                  ...
                  <Text style={[styles.courtName, { flex: 1 }, selectedCourt === c.id && { color: colors.primary }]}>
                    {c.name}
                  </Text>
                  {selectedCourt === c.id && (
